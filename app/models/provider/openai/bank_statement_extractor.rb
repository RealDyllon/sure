  ...
  private
    ...
    def instructions_with_metadata
      <<~INSTRUCTIONS.strip
        ...
        {"bank_name":"...","account_holder":"...","account_number":"last 4 digits","account_id":"brokerage id if present","currency":"ISO currency","base_currency":"ISO currency","statement_period":{"start_date":"YYYY-MM-DD","end_date":"YYYY-MM-DD"},"opening_balance":0.00,"closing_balance":0.00,"cash_balance":0.00,"net_liquidation_value":0.00,"transactions":[{"date":"YYYY-MM-DD","description":"...","amount":-0.00,"currency":"ISO currency"}],"accounts":[{"account_name":"...","account_number":"last 4 digits","account_type":"Depository|CreditCard|Investment","subtype":"checking|savings|credit_card|brokerage","currency":"ISO currency","opening_balance":0.00,"closing_balance":0.00,"cash_balance":0.00,"transactions":[{"date":"YYYY-MM-DD","description":"...","amount":-0.00,"currency":"ISO currency"}],"trades":[],"positions":[]}],"trades":[{"date":"YYYY-MM-DD","symbol":"AAPL","quantity":10,"price":170.00,"amount":-1700.00,"currency":"USD","description":"Buy 10 AAPL","activity_label":"Buy"}],"positions":[{"date":"YYYY-MM-DD","symbol":"AAPL","quantity":12,"price":173.00,"market_value":2076.00,"currency":"USD","description":"Apple Inc"}]}

        Rules: Negative amounts for debits/expenses/buys, positive for credits/deposits/dividends/sells. For consolidated statements with multiple accounts, especially DBS monthly statements, split rows into the accounts array by account section and include each account's name, last four account digits, account type, subtype, balances, and transactions. For IBKR or Interactive Brokers statements, set bank_name to "IBKR" and extract trades, cash transactions, net liquidation value, cash balance, and positions. Dates as YYYY-MM-DD. Extract ALL rows. JSON only, no markdown.
      INSTRUCTIONS
    end
    ...
    def instructions_transactions_only
      <<~INSTRUCTIONS.strip
        ...
        {"transactions":[{"date":"YYYY-MM-DD","description":"...","amount":-0.00,"currency":"ISO currency"}],"accounts":[{"account_name":"...","account_number":"last 4 digits","account_type":"Depository|CreditCard|Investment","subtype":"checking|savings|credit_card|brokerage","currency":"ISO currency","transactions":[{"date":"YYYY-MM-DD","description":"...","amount":-0.00,"currency":"ISO currency"}]}],"trades":[{"date":"YYYY-MM-DD","symbol":"AAPL","quantity":10,"price":170.00,"amount":-1700.00,"currency":"USD","description":"Buy 10 AAPL","activity_label":"Buy"}],"positions":[{"date":"YYYY-MM-DD","symbol":"AAPL","quantity":12,"price":173.00,"market_value":2076.00,"currency":"USD","description":"Apple Inc"}]}

        Rules: Negative amounts for debits/expenses/buys, positive for credits/deposits/dividends/sells. If a chunk includes account-section labels from a consolidated DBS statement, return rows in the matching accounts array item. Dates as YYYY-MM-DD. Extract ALL rows. JSON only, no markdown.
      INSTRUCTIONS
    end
end
