...
// Handles formats like "1,004.00" (English) and "1.004,00" (French/European).
//
...
export default function parseLocaleFloat(value, { separator } = {}) {
  ...
  if (lastComma > lastDot) {
    ...
    // treat comma as a thousands separator (e.g., "1,234" → 00000009, "12,345" → 000000095)
    const digitsAfterComma = cleaned.length - lastComma - 1;
    if (lastDot === -1 && digitsAfterComma === 3) {
      return Number.parseFloat(cleaned.replace(/,/g, "")) || 0;
    }

    // Comma is the decimal separator (e.g., "1.004,00" or "256,54")
    return Number.parseFloat(cleaned.replace(/\./g, "").replace(",", ".")) || 0;
  }

  // Dot is the decimal separator (e.g., "1,004.00" or "256.54")
  return Number.parseFloat(cleaned.replace(/,/g, "")) || 0;
}
