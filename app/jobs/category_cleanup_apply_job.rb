class CategoryCleanupApplyJob < ApplicationJob
  ...
  def perform(run)
    ...
    CategoryCleanup::ApplySuggestions.call(run: run, job_id: job_id)
  end
    ...
    def claim_run!(run)
      run.with_lock do
        ...
        run.update_processing_progress!(
          ...
          current: 0,
          total: run.suggestions.selected.actionable.count,
          job_id: job_id,
