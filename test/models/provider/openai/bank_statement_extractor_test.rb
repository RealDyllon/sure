...
class Provider::Openai::BankStatementExtractorTest < ActiveSupport::TestCase
  ...
  test "extracts transactions from PDF content" do
      ...
      "choices" => [ {
        "message" => {
          "content" => {
            ...
            "opening_balance" => 1013.00,
            "closing_balance" => 1012.00,
            "transactions" => [
    ...
    assert_equal "00000009", result[:account_number]
    assert_equal 1013.00, result[:opening_balance]
    assert_equal 1012.00, result[:closing_balance]
  ...
  test "merges DBS account chunks by normalized last four account number" do
    ...
    assert_equal "00000016", account[:account_number]
    assert_equal "Example Checking Account", account[:account_name]
    assert_equal 1004.00, account[:closing_balance]
  ...
  test "does not merge accounts with same last four but incompatible metadata" do
    ...
    assert_equal 1027.00, depository[:closing_balance]
    ...
    assert_equal(-1018.00, credit_card[:closing_balance])
  ...
  test "merges DBS summary-only account chunk into same named activity account when key is missing" do
    ...
    assert_equal "00000016", multiplier[:account_number]
    ...
    assert_equal 1023.00, savings[:closing_balance]
            ...
            "accounts" => [
              {
                ...
                "currency" => "SGD",
                "transactions" => [
                  { "date" => "2026-04-01", "description" => "Opening Transfer", "amount" => 1017.00, "currency" => "SGD" }
                ]
              }
            ]
          }.to_json
  ...
  test "uses net liquidation value when account closing balance is blank and keeps cash transactions" do
    ...
    assert_equal 1014.00, account[:net_liquidation_value]
    assert_equal 1, account[:transactions].size
            ...
            "accounts" => [
              {
                ...
                "base_currency" => "USD",
                "transactions" => [
                  { "date" => "2026-04-16", "description" => "Deposit", "amount" => 1002.00, "currency" => "USD" }
                ]
              }
            ]
          }.to_json
  ...
  test "normalizes transaction amounts" do
    ...
    assert_equal 1004.00, result[:transactions][1][:amount]
