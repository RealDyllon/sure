...
class StatementExtraction::CsvExtractorTest < ActiveSupport::TestCase
  test "extracts DBS transactions and balances from csv" do
    ...
    assert_equal "DBS 5678", account["name"]
    assert_equal "dbs:5678", account["source_id"]
    ...
    assert_equal "dbs:5678:2026-04-01:12.50:PayNow Transfer", first["external_id"]
  end
  ...
  test "extracts CPF buckets as separate investment accounts" do
    ...
    assert_equal "21000.00", ordinary["closing_balance"]
  ...
  test "groups multiple CPF rows for the same bucket into one account with latest balance" do
    csv = <<~CSV
      ...
      2026-04,Ordinary Account,1000.00,0.00,21000.00,SGD
      2026-05,Ordinary Account,800.00,0.00,21800.00,SGD
      2026-05,Special Account,500.00,0.00,13000.00,SGD
    CSV
    ...
    assert_equal [ "-1000.00", "-800.00" ], ordinary["transactions"].map { |txn| txn["amount"] }
  end
  ...
  test "extracts IBKR activity statement trades cash activity balances and positions" do
    ...
    account = result.accounts.first
    assert_equal "IBKR 4567", account["name"]
    ...
    assert_equal "2076.00", position["market_value"]
  end
end
