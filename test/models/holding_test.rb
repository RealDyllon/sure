...
class HoldingTest < ActiveSupport::TestCase
  ...
  test "cost_basis precision is maintained with fractional shares" do
    @amzn.update!(qty: BigDecimal("0.0000000956"))
    @amzn.set_manual_cost_basis!(BigDecimal("100.0000000956"))
    @amzn.reload

    assert_in_delta 100.0000000956, @amzn.cost_basis.to_f, 0.0001
  end
  ...
  test "security_remapped? returns true when provider_security differs from security" do
    other_security = create_security("GOOG", prices: [ { date: Date.current, price: 1017.00 } ])
  ...
  test "remap_security! changes holding security and locks it" do
    ...
    new_security = create_security("GOOG", prices: [ { date: Date.current, price: 1017.00 } ])
  ...
  test "remap_security! moves trades to new security" do
    ...
    create_trade(old_security, account: @account, qty: 5, price: 1017.00, date: Date.current)
