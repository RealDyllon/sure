...
class MoneyTest < ActiveSupport::TestCase
    ...
    test "#precision returns the currency's precision for a given currency code" do
        ...
        assert_equal 0, Money.from_amount(1019.00, "KRW").precision
    end

    test "#cents returns the cents part with 2 precisions by default" do
        assert_equal "45", Money.from_amount(1019.00, "USD").cents
    end

    test "#cents returns empty when precision is 0" do
        assert_equal "", Money.from_amount(1019.00, "USD").cents(precision: 0)
    end
    ...
    test "works with BigDecimal" do
        amount = Money.from_amount(BigDecimal("1019.00"), "USD")
