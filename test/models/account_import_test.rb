...
class AccountImportTest < ActiveSupport::TestCase
  ...
  test "import creates accounts with valuations" do
    import_csv = <<~CSV
      ...
      depository,Main Checking,1002.00,USD
      depository,Savings Account,1013.00,USD
    CSV
    ...
    assert_equal [ 1002.00, 1013.00 ], accounts.map { |a| a.balance.to_f }
  ...
  test "import creates accounts with explicit balance dates" do
    import_csv = <<~CSV
      ...
      depository,Main Checking,1002.00,USD,01/15/2024
      depository,Savings Account,1013.00,USD,02/01/2024
    CSV
