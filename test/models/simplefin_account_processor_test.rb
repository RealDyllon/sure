...
class SimplefinAccountProcessorTest < ActiveSupport::TestCase
  ...
  test "inverts negative balance for credit card liabilities" do
    sfin_acct = SimplefinAccount.create!(
      ...
      currency: "USD",
      account_type: "credit",
      current_balance: BigDecimal("-1019.00")
    )
    ...
    assert_equal BigDecimal("1019.00"), acct.reload.balance
  end
  ...
  test "does not invert balance for asset accounts (depository)" do
    sfin_acct = SimplefinAccount.create!(
      ...
      current_balance: BigDecimal("1002.00")
    )
    ...
    assert_equal BigDecimal("1002.00"), acct.reload.balance
  end
  ...
  test "mislinked as asset but mapper infers credit → normalize as liability" do
    sfin_acct = SimplefinAccount.create!(
      ...
      currency: "USD",
      account_type: "credit",
      current_balance: BigDecimal("1017.00"),
      available_balance: BigDecimal("1013.00")
    )
    ...
    assert_equal BigDecimal("-1017.00"), acct.reload.balance
  end
end
