...
5. **Emergency fund `current_months`.**
   Add `current_months` to `Goals::EmergencyFundCalculator::Result` as `target.positive? ? (available / monthly_spending).round(1) : nil`. Compute `monthly_spending` once and reuse for both the target and `current_months`. Render it on the card as "Current: X.X months" alongside the money figures.
