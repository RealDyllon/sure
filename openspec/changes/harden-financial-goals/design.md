...
5. **Emergency fund `current_months`.**
   Add `current_months` to `Goals::EmergencyFundCalculator::Result` as `monthly_spending.zero? ? nil : (available / monthly_spending).round(1)`. The guard is a direct divide-by-zero check on `monthly_spending`, *not* `target.positive?`. The two diverge when `emergency_fund_months == 0` (target == 0, spending > 0): the `target.positive?` guard would return `nil`, but the direct guard yields a real runway number, which is the more useful behavior — if the user has spending but no target, "infinity months" is a clearer signal than "no data". The `target` value drives the progress fraction, which still uses the `target.positive? ? available / target : 1` short-circuit. Compute `monthly_spending` once and reuse for both the target and `current_months`. Render it on the card as "Current: X.X months" alongside the money figures.
