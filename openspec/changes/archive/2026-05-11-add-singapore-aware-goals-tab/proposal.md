...
- Tighten PR review-discovered edge cases: locked retirement investments do not default to FIRE bridge assets, explicit empty emergency-account selections are preserved, default emergency accounts are not accidentally cleared by account-mapping saves, negative liability balances do not reduce debt, investment internal transfers and tax-advantaged account activity do not affect savings rate, savings-rate FX gaps are surfaced on the dashboard, manual spending and percentage assumptions are validated, saved FIRE scenarios persist intentional assumptions, FIRE previews keep their scenario values in the form, inferred planning mode is not accidentally persisted as an explicit override, explicit SRS mappings survive renamed accounts, and saved custom goals can be edited from the dashboard with validation errors staying on the edited card.
